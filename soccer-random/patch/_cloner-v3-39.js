...
(function() {
    ...
    const CDN_BASE= "https://cdn.jsdelivr.net/gh/ahrdevelopers/soccer-random/";
    ...
    const URLPatch= function(url, note= "", type= "") {
        ...
        const isCdnAsset= p => p.startsWith("images/") || p.startsWith("media/") || p.startsWith("box2d.wasm");
        const relAssetPath= url.replace(/^(?:\.\/|\/|\.\.\/)+/, "");
        if (isCdnAsset(relAssetPath)) {
            ClonerLog("URLPatch", note, url, "→", `${CDN_BASE}${relAssetPath}`);
            return `${CDN_BASE}${relAssetPath}`;
        }
        if (url.startsWith(GAME_PATH)) {
            const localAssetPath= url.slice(GAME_PATH.length).replace(/^\/+/, "");
            if (isCdnAsset(localAssetPath)) {
                ClonerLog("URLPatch", note, url, "→", `${CDN_BASE}${localAssetPath}`);
                return `${CDN_BASE}${localAssetPath}`;
            }
            return url;
        }
        ...
        if (url.startsWith("https://cdn.jsdelivr.net/gh/ahrdevelopers/soccer-random/")) {
            return url;
        }
        ...
        for (const [matchUrl, replaceUrl] of Object.entries(REPLACEs)) {
            if (url.startsWith(matchUrl)) {
                ...
                if (replace_url.startsWith("./patch/")) {
                    replace_url= `${CDN_BASE}${replace_url.replace("./", "")}`;
                }
        ...
        if (url.startsWith(`${SCHEME}://${HOST}/cdn-cgi/`)) {
            return `${CDN_BASE}patch/blocked.json`;
        }
        if (type== "JSON") {
            return `${CDN_BASE}patch/blocked.json`;
        }
